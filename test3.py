...
UPDATE_INTERVAL = 0.5  # Time in seconds between JSON updates
...
# --- Parsing Functions (Your existing logic, now in a reusable function) ---
...
def update_json():
    ...
    # NEW: Create a sorted list of top players for the JSON output
...
def end_of_match_processing(match_data, target_teams_dict):
    ...
    # NEW: Aggregate player data after each match with new stats
    ...
    for player_id, player_data in all_players_in_match.items():
        ...
        tournament_data["tournament_players"][player_id]["totalDamage"] += player_data.get("damage", 0) # NEW
        tournament_data["tournament_players"][player_id]["totalKnockouts"] += player_data.get("knockouts", 0) # NEW
...
def main():
    ...
    # 2. Get the current log file to begin live monitoring
    current_logs = get_current_round_logs()
    if not current_logs:
        print("🔴 No current log files found. Exiting.")
        return
    
    LIVE_LOG_FILE = current_logs[-1] # Assume the newest file is the live one

    # 3. Process the last known state of the live log file for initial load
    print(f"🚀 Starting initial processing of live log file: {LIVE_LOG_FILE}...")
    try:
        with open(LIVE_LOG_FILE, 'r', encoding='utf-8') as f:
            log_content = f.read()
            processed_data = process_log_data(log_content, tournament_data["current_match"])
            print("✅ Initial processing complete.")
            print(f"Found {len(processed_data['teams'])} teams")
            print(f"Found {len(processed_data['players'])} active players")
            print(f"Found {len(processed_data['eliminated_players'])} eliminated players")
    except FileNotFoundError:
        print(f"Error: Live log file not found at {LIVE_LOG_FILE}. Exiting.")
        return
    
    update_json()

    # 4. Start the web server after initial JSON is ready
    webserver.start_server()
    
    print("\n✅ JSON file has been updated. Starting live monitoring for new updates. Press Ctrl+C to stop.")
    
    last_update_time = time.time()
    # print(json.dumps(tournament_data["tournament_players"], indent=2))

    try:
        with open(LIVE_LOG_FILE, 'r', encoding='utf-8') as f:
            f.seek(0, 2) # Move to the end of the file for live monitoring
            ...
            while True:
                ...
                if "TotalPlayerList:" in line or "TeamInfoList:" in line:
                    with open(LIVE_LOG_FILE, 'r', encoding='utf-8') as temp_f:
                        log_content = temp_f.read()
                        process_log_data(log_content, tournament_data["current_match"])
